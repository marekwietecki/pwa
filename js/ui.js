...
const AVATAR_PLACEHOLDER_SVG =
  '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-user-round preview-icon"><circle cx="12" cy="8" r="5"/><path d="M20 21a8 8 0 0 0-16 0"/></svg>';
...
export const UI = {
  ...
  updateUserHeader: async (AppState) => {
    ...
    if (avatarLabel) {
      if (stats.avatar) avatarLabel.textContent = stats.avatar;
      else avatarLabel.innerHTML = AVATAR_PLACEHOLDER_SVG;
    }
  ...
  openAvatarPicker: async () => {
    ...
    const stats = await DataManager.getUserStats();
    const currentAvatar = stats.avatar || null;
